...
\begin{frame}{Quais são as variáveis?}
  ...
  \begin{block}{Primeira alternativa}
    \begin{enumerate}
    \item Queremos ``explicar'' a ``relação'' entre o hormônio de $x_2$ sabendo o hormônio de $x_1$
...
\begin{frame}{Quais são as variáveis?}
  ...
  \begin{block}{Segunda alternativa (método da aula passada)}
    \begin{enumerate}
    \item Queremos ``explicar'' a ``relação'' entre a diferença e a referência ({\bf zero})
    \end{enumerate}
  \end{block}
  \begin{block}{Esta relação pode ser expressa como}
    \begin{displaymath}
...
\begin{frame}{Uma breve interrupção para mini-pânico}
  ...
  \begin{block}{Estratégia proposta}
    Para considerar a diferença, precisamos do {\em SEM da diferença}.
  \end{block}
\end{frame}
...
\begin{frame}{Uma breve interrupção para mini-pânico}
  ...
  \begin{block}{Mais precisamente}
    \begin{displaymath}
      \text{horm. grávid.} = \text{horm. não grávid.} + \text{Erro}_1 + \text{Erro}_2
    \end{displaymath}
  \end{block}
...
\begin{frame}{De volta à programação normal}
  \begin{center}
    \includegraphics[height=.9\textheight]{Cap7/naoentreempanico}
  \end{center}
\end{frame}
