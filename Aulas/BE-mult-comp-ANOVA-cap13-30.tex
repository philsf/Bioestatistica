...
\subtitle
{\small Teste paramétrico para vários grupos (desfecho quantitativo)} % (optional)
...
\begin{frame}[label=requisito]{\scriptsize Como comparar médias}
  ...
  \item Assumindo que atendemos às premissas do teste t, consideramos:
    \begin{itemize}
      \footnotesize
    \item variabilidade dos grupos\footnote{\tiny Se possível, semelhantes. Caso contrário, correção de Welch.}
    \item tamanho do estudo (n)\footnote{\tiny Componente do DP e do SEM. Usado como GL para o t crítico.}
    \end{itemize}
  \end{itemize}
  \bigskip
...
\begin{frame}{\scriptsize }
  ...
  \item<3-> Testamos simultaneamente três\footnote{\tiny Leia várias vezes o Capítulo 13!}
...
\begin{frame}{\scriptsize Pensar é obrigatório}
  ...
  \item {\em Só o pesquisador sabe.}
    % \smallskip
  \item A metodologia da análise precisa levar em conta todo o planejamento do estudo\footnote{\tiny Leia várias vezes o Capítulo 13!}.
  \end{itemize}
  \bigskip
  \bigskip
  \bigskip
...
\begin{frame}{\scriptsize Componentes da One-Way ANOVA}
  ...
  \vfill
  \footnotesize
  Hipótese: $\varepsilon$ é um erro aleatório \footnote{\tiny residual -- não é explicado pela relação entre as variáveis do modelo} normalmente distribuído e centrado em zero -- a incerteza que não pode ser controlada.
\end{frame}
...
\begin{frame}{\scriptsize Expectativa x realidade\footnote{\tiny Mesma ideia do qui-quadrado.} -- O teste F\footnote{\tiny O teste leva em conta dois graus de liberdade: numerador e denominador}}
  \begin{block}{}
    \footnotesize
...
\begin{frame}{\scriptsize Testes {\em post-hoc}}
  \begin{itemize}
    \footnotesize
  \item O teste de ANOVA é apenas a primeira parte!\footnote{\tiny Está com saudade do teste t?}
    \bigskip
    \bigskip
    \begin{block}{}
...
%  \item O teste de Tukey tem mais especificidade, mas menos sensibilidade
    \bigskip
    \bigskip
 \item O ajuste de Bonferroni multiplica o p-valor\footnote{\tiny ou, analogamente, divide o nível de significância} pelo número de comparações, mas seus IC são muito grandes
    \bigskip
...
\begin{frame}{\scriptsize Exemplo 13.5 -- testes t com ajuste de Tukey}
  \begin{exampleblock}{Exemplo 13.5}
...
\begin{frame}{\scriptsize ANOVA dois parâmetros}
  ...
  \item Vimos como usar o ANOVA com {\bf uma} var. independente categórica
    \bigskip
  \item O teste ANOVA permite qualquer quantidade de variáveis independentes! E de qualquer tipo\footnote{\tiny Na verdade, ANOVA e Regressão Linear são Múltipla são siameses}
    \bigskip
    \bigskip
...
\againframe{ident-visual}

\againframe{cenario1}

\begin{frame}[label=cenario3]{\scriptsize Cenário 3 -- esses 3 grupos têm médias diferentes?}
  \begin{center}
    \includegraphics[height=.9\textheight]{Cap13-30/cenario12}
  \end{center}
\end{frame}

\begin{frame}{\scriptsize Cenário 3 -- esses 3 grupos têm médias diferentes?}
  \begin{center}
    \includegraphics[height=.6\textheight]{Cap13-30/cenario11_medias}
    \includegraphics[height=.6\textheight]{Cap13-30/cenario12_medias}
  \end{center}
\end{frame}
\againframe{cenario2}

\begin{frame}[label=cenario4]{\scriptsize Cenário 4 -- esses 3 grupos têm médias diferentes?}
  \begin{center}
    \includegraphics[height=.9\textheight]{Cap13-30/cenario22}
  \end{center}
\end{frame}

\begin{frame}{\scriptsize Cenário 4 -- esses 3 grupos têm médias diferentes?}
  \begin{center}
    \includegraphics[height=.6\textheight]{Cap13-30/cenario21_medias}
    \includegraphics[height=.6\textheight]{Cap13-30/cenario22_medias}
  \end{center}
\end{frame}
...
\againframe{exercicio-enunciado}
...
\begin{frame}{\scriptsize Cenário 1 -- Solução}
  \begin{columns}
    \begin{column}{5cm}
      \begin{exampleblock}{\footnotesize P-valores dos 3 testes t}
        \begin{enumerate}
          \tiny
        \item \sout{Placebo x Trat. A $\Rightarrow p=0.025$}
        \item \sout{Placebo x Trat. B $\Rightarrow p=0.100$}
        \item \sout{Trat. A x Trat. B $\Rightarrow p=0.876$}
        \end{enumerate}
      \end{exampleblock}
      \bigskip
      \bigskip
      \begin{exampleblock}{\footnotesize P-valores ajustados (Bonferroni)}
        \begin{enumerate}
          \tiny
        \item \alert{Placebo x Trat. A $\Rightarrow p=0.076$}
        \item Placebo x Trat. B $\Rightarrow p=0.299$
        \item Trat. A x Trat. B $\Rightarrow p=1.000$
        \end{enumerate}
      \end{exampleblock}
    \end{column}
    \begin{column}{5cm}
...
\begin{frame}{\scriptsize Cenário 2 -- Solução}
  \begin{columns}
    \begin{column}{5cm}
      \begin{exampleblock}{\footnotesize P-valores dos 3 testes t}
        \begin{enumerate}
          \tiny
        \item \sout{Placebo x Trat. A $\Rightarrow p< 0.0001$}
        \item \sout{Placebo x Trat. B $\Rightarrow p=0.00037$}
        \item \sout{Trat. A x Trat. B $\Rightarrow p=0.02943$}
        \end{enumerate}
      \end{exampleblock}
      \bigskip
      \bigskip
      \begin{exampleblock}{\footnotesize  P-valores ajustados (Bonferroni)}
        \begin{enumerate}
          \tiny
        \item Placebo x Trat. A $\Rightarrow p< 0.0001$
        \item Placebo x Trat. B $\Rightarrow p=0.0011$
        \item \alert{Trat. A x Trat. B $\Rightarrow p=0.0883$}
        \end{enumerate}
      \end{exampleblock}
    \end{column}
    \begin{column}{5cm}
...
\begin{frame}{\scriptsize Resumo}
  ...
  \item Vimos o modelo {\bf ANOVA com fatores fixos} para comparar médias
    \bigskip
  \item Há também...\footnote{\tiny Todos fora do escopo deste curso}
    \begin{itemize}
      \scriptsize
    \item<2-> {\bf ANOVA com interações entre os fatores}
    \item<2-> {\bf ANOVA com Medidas Repetidas} quando você mensura do participante em vários momentos diferentes\footnote{\tiny RM ANOVA é mencionado no livro-texto}
  ...
  \item<2-> ANOVA é a base para (livros de) Design of Experiments (DoE)
    \bigskip
  \item<2-> Considere sempre usar {\bf desenhos balanceados}\footnote{\tiny Grupos com {\bf mesmo tamanho}}!
  \end{itemize}
\end{frame}
