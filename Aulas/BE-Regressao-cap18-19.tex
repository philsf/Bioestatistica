...
\begin{frame}{Reta de regressão}
  ...
  \item Dados observados: Y (desfecho) e X (preditor)
...
\begin{frame}[fragile]{Diagnosticando a regressão}
  ...
  \bigskip
    \begin{exampleblock}{Resposta}
      \begin{itemize}
      \item Decréscimo de 3 unidades de BMD

        \bigskip
        \scriptsize (para cada aumento unitário de BMI)
    \end{itemize}
  \end{exampleblock}
  \vfill
  \begin{exampleblock}{Saída típica de um programa de análise}
...
\begin{frame}{\small Dispersão em torno da reta {\bf cresce} ao longo da faixa}
  \begin{center}
    \includegraphics[height=.9\textheight]{Cap18-19/pratica-plot-heterocedasticidade}
  \end{center}
\end{frame}

\begin{frame}{\small Dispersão em torno da reta {\bf varia} ao longo da faixa}
  \begin{center}
