...
\begin{frame}{Revisão: equação da reta}
  ...
  \begin{block}{}
    \begin{itemize}
    \item<2-> Note que aqui os ``dados'' já foram coletados (fixos)
    \item<2-> Nosso objetivo é estimar os parâmetros da reta \alert{$b$} e \alert{$a$}
    \end{itemize}
  \end{block}
\end{frame}
...
\begin{frame}{Reta de regressão}
  ...
  \item Dados observados: X (preditor) e Y (desfecho)
  \item Parâmetros estimados (a e b)
  \end{itemize}}
  \vfill
