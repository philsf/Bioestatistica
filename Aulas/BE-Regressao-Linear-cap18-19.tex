...
\begin{frame}{\scriptsize Quais são as variáveis?}
  ...
  \item Independente: BMI (contínua)
    \begin{itemize}
      \scriptsize
    \item Sinônimos\footnote{\tiny Em alguns contextos também covariável/cofator (quando há mais de uma V.I.)}: preditor, fator
    \end{itemize}
  \end{itemize}
  \vfill
...
\begin{frame}{\scriptsize Interpretação dos parâmetros da reta}
  ...
  \item A inclinação é quanto BMD altera\footnote{\scriptsize na média!} quando aumentamos o BMI em 1 unidade
  \end{itemize}
  \vfill
  \begin{block}{Atenção}
    \footnotesize
...
\begin{frame}{\scriptsize Reta de regressão}
  ...
  \item Minimiza os resíduos (erros aleatórios $\varepsilon$)\footnote{\tiny Método dos mínimos quadrados}
...
\begin{frame}{\scriptsize Análise de Regressão}
  ...
  \begin{center}
    \footnotesize
    Uma forma \alert{simplista} de aferir a qualidade do ajuste do modelo\footnote{\tiny Também chamada de Goodness of Fit (GoF)} é o Coeficiente de Determinação $r^2$.
...
\begin{frame}[fragile]{\scriptsize Diagnosticando a regressão\footnote{\tiny $H_a$: Intercepto não é igual a 0 (modelo nulo)}}
  \begin{block}{Perguntas}
    \footnotesize
    \begin{itemize}
      \footnotesize
...
\begin{frame}[fragile]{\scriptsize Diagnosticando a regressão\footnote{\tiny $H_a$: Inclinação não é igual a 0 (modelo nulo)}}
  \begin{block}{Perguntas}
    \footnotesize
    \begin{itemize}
      \footnotesize
...
\begin{frame}{\scriptsize Regressão linear simples x múltipla\footnote{\tiny Ajustar para outros fatores: como visto no abstract do exercício!}}
