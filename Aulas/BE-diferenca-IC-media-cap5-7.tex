...
\begin{frame}{\scriptsize Recapitulando}
  ...
  \begin{block}{Nomenclatura}<2->
    \small
    A distribuição {\em Normal Padrão} também é chamada de {\bf distribuição Z}.
  \end{block}
\end{frame}
...
\begin{frame}{\scriptsize A distribuição t de Student}
  ...
  \item Student (pseudônimo de W. S. Gossett [1876-1937]\footnote{trabalhando
    ...
    \bigskip
  \item Distribuição {\bf t} ({\tiny baseada na distribuição Normal})
    \bigskip
  \item Melhor se aproxima dos dados de {\em amostras pequenas}
    \bigskip
 % \item Semelhante à Normal padrão (Z), mas tem variância maior
 %    \bigskip
  \item 3$^o$ parâmetro \alert{graus de liberdade}\footnote{df em inglês} {\tiny vinculado ao tamanho da amostra $n$}.
    % \begin{displaymath}
    %   gl = n-1
    % \end{displaymath}
  \end{itemize}
...
\begin{frame}{\scriptsize Propriedades da distribuição t}
  ...
  \begin{block}{Isto é}
    \footnotesize
    Quanto mais graus de liberdade, mais a distribuição $t$ se parece com a distribuição Normal padrão ($Z$)
  \end{block}
    \bigskip
  \begin{block}{Pense...}<2->
    \footnotesize
...
\begin{frame}{\scriptsize Tamanho do efeito}
  \begin{center}
    \footnotesize
    Nesse caso a diferença média ($\bar{x_d}$) é o \alert{efeito observado}

    \bigskip
    \uncover<2->{Efeitos grandes são mais fáceis de ser detectados}
  \end{center}
\end{frame}
