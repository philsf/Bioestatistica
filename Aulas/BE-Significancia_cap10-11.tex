...
\begin{frame}{Identificando hipóteses}
  \begin{block}{1 teste = 2 hipóteses}
    Um teste de hipóteses envolve a formulação de uma \alert{hipótese nula} e uma \alert{hipótese alternativa}.
  \end{block}
  \begin{itemize}
    \small
  \item A hipótese nula ($H_0$) é a hipótese que não há efeito real.
  \item A hipótese alternativa ($H_1$ ou $H_a$) é a hipótese de há efeito real \alert{que pode ser detectado} com o experimento
    \begin{itemize}
    \item Hipótese de interesse científico
    \end{itemize}
.
  \end{itemize}
\end{frame}
