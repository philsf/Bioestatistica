...
\begin{frame}{Exemplo 1}
  \begin{exampleblock}{Exemplo 1}
    ...
    % \item $Z = \frac{1.05 - 1.2}{\frac{0.5}{\sqrt{100}}} = -3$
      \begin{itemize}
      \item O teste Z retorna $p = 0.0027$ (para $n$ grande)
      \item O teste t retorna $p = 0.0034$ (em geral)
      \end{itemize}
    \item<2-> Como $p < 0.05$, concluímos que há evidência para rejeitar
      $H_0$.
    \item<2-> {\bf Resultado: O tempo de resposta médio é \alert{significativamente} diferente de 1.2 s.}
    \item<2-> {\bf Conclusão: \uncover<3->{há evidências que a droga altera o tempo (...)  de resposta (...)}}
    \end{itemize}
  \end{exampleblock}
