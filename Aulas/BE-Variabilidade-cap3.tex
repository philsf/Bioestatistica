...
\begin{frame}[fragile]{\scriptsize Qual é a diferença?}
  \begin{block}{}
    \footnotesize
    \centering
    O que acontece com a média, na presença de um valor extremo?

    ({\tiny muito grande, ou muito pequeno em relação aos outros})
  \end{block}
  \visible<2->{
  \begin{exampleblock}{Colesterol}
    \bigskip
...
\begin{frame}{\scriptsize Atenção!}
  \footnotesize
  Valores extremos podem ser:

  \bigskip
  \begin{itemize}
    \footnotesize
  \item erros de digitação/tabulação
    \medskip
  \item erros de mensuração/instrumentais
    \medskip
  \item valores legítimos, porém raros
    \medskip
  \item valores legítimos.

    ({\tiny i.e., apenas {\em parecem} extremos, mas não são})
  \end{itemize}
\end{frame}
