...
\begin{frame}{\scriptsize Medida de dispersão de dados não-paramétricos}
  \begin{block}{Descrição de dados amostrais}
    ...
    \item Dados ``bem comportados''\footnote{\scriptsize paramétricos: veremos o que isso significa em aulas futuras}
      \begin{itemize}
        \scriptsize
      \item Média e DP ( $\bar{x} \pm s$ )
      \item Média e Erro Padrão da Média ($\bar{x} \pm $ SEM {\tiny em inglês})
      \end{itemize}
      \bigskip
    \item Dados ``mal comportados''
      \begin{itemize}
        \scriptsize
      \item Mediana e Amplitude InterQuartílica ($M_d$ e AIQ)
      \item Mediana e Intervalo InterQuartílico ($M_d$ e IQR {\tiny em inglês})
      \end{itemize}
    \end{itemize}
  \end{block}
\end{frame}
